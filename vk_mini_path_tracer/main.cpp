...
int main(int argc, const char** argv)
{
  ...
  // Create a buffer to store the rays
  VkDeviceSize rayBufferSizeBytes = render_width * render_height * sizeof(float) * 3 * 2; // origin and direction per pixel
  VkBufferCreateInfo rayBufferCreateInfo = nvvk::make<VkBufferCreateInfo>();
  rayBufferCreateInfo.size = rayBufferSizeBytes;
  rayBufferCreateInfo.usage = VK_BUFFER_USAGE_STORAGE_BUFFER_BIT | VK_BUFFER_USAGE_TRANSFER_DST_BIT;
  nvvk::Buffer rayBuffer = allocator.createBuffer(rayBufferCreateInfo,
      VK_MEMORY_PROPERTY_HOST_VISIBLE_BIT
      | VK_MEMORY_PROPERTY_HOST_CACHED_BIT
      | VK_MEMORY_PROPERTY_HOST_COHERENT_BIT);
  ...
  for (int i = 0; i < 4; ++i)
  {
      pipelines[i] = CreateComputePipeline(context, modules[i], generateDescriptorSetContainer.getLayout());
  }
  ...
  for (int phase = 0; phase < 4; ++phase)
  {
      ...
      // Bind the descriptor set
      VkDescriptorSet descriptorSet = generateDescriptorSetContainer.getSet(phase);
      vkCmdBindDescriptorSets(cmdBuffer, VK_PIPELINE_BIND_POINT_COMPUTE, generateDescriptorSetContainer.getPipeLayout(), 0, 1,
          &descriptorSet, 0, nullptr);
      ...
      //// Add a command to make shader writes available to read from the CPU
      //VkMemoryBarrier generateMemoryBarrier = nvvk::make<VkMemoryBarrier>();
      //generateMemoryBarrier.srcAccessMask = VK_ACCESS_SHADER_WRITE_BIT;
      //generateMemoryBarrier.dstAccessMask = VK_ACCESS_HOST_READ_BIT;
      //vkCmdPipelineBarrier(cmdBuffer,
      //    VK_PIPELINE_STAGE_COMPUTE_SHADER_BIT,
      //    VK_PIPELINE_STAGE_HOST_BIT,
      //    0,
      //    1, &generateMemoryBarrier,
      //    0, nullptr, 0, nullptr);
      ...
      // Map the buffer
      void* rayMappedData = allocator.map(rayBuffer);
      // reinterpret the buffer as vec3
      glm::vec3* rayBufferData = reinterpret_cast<glm::vec3*>(rayMappedData);
      // Create an array of RayData
      std::vector<RayData> rayDataArray(render_width * render_height);

      // Copy the data from the mapped region to the array
      for (uint32_t i = 0; i < render_width * render_height; ++i)
      {
          rayDataArray[i].origin = rayBufferData[i * 2];
          rayDataArray[i].direction = rayBufferData[i * 2 + 1];
      }

      allocator.unmap(rayBuffer);
  }
